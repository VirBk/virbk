...
- 2026-09-20 F34 landed as a control-plane records commit, not a writer envelope. AGENTS section 8 says the control plane writes documentation directly, section 3 says a records envelope gets no review, and T53 says an archive-at-80-per-cent cut is a records commit between landings and does not issue a writer. Three changes. AGENTS section 3 now carries D-53's orchestration model: a lane carries a wave and a depends-on and order is derived from that graph rather than judged, siblings run only on holds proved disjoint at the files and written down with a free slot not counting as proof, one review slot stays reserved, and a seat census is read from the process table with a dead seat resumed on its own session id. The provisional group of D-49 moved to factory/OPERATING_MODEL.md with a table naming what enforces each bullet, checked at the objects: seven of the nine map to a trap in the digest every packet carries, and 'a writer never fast-forwards main' is already AGENTS section 3's push the branch only, never main, so the move removed a duplicate rather than a law. Section 9's index kept the twelve rows a seat or an issuing control plane reaches for constantly and the rest moved verbatim. Measured: node factory/tools/sizeBudget.mjs puts AGENTS.md at 86.6 per cent of its 12 KB cap, from 96.8, and the seat packet at 93.5 from 98.0; node factory/tools/packet.mjs cost puts the control-plane packet at 34.9 KB, out of the band entirely. factory/OPERATING_MODEL.md gained a 16 KB cap, because a file that receives what a capped file sheds and is itself unmeasured is T01.
